...
test("S381 EcoCash operationId matches disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.equal(
    served.paths["/api/webhooks/ecocash"]?.post?.operationId,
    disk.paths["/api/webhooks/ecocash"]?.post?.operationId,
  );
  assert.equal(
    served.paths["/api/webhooks/ecocash"]?.post?.operationId,
    "webhookEcocash",
...
test("S382 PayPal operationId matches disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.equal(
    served.paths["/api/webhooks/paypal"]?.post?.operationId,
    disk.paths["/api/webhooks/paypal"]?.post?.operationId,
  );
  assert.equal(
    served.paths["/api/webhooks/paypal"]?.post?.operationId,
    "webhookPaypal",
...
test("S383 Escrow operationId matches disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.equal(
    served.paths["/api/webhooks/escrow"]?.post?.operationId,
    disk.paths["/api/webhooks/escrow"]?.post?.operationId,
  );
  assert.equal(
    served.paths["/api/webhooks/escrow"]?.post?.operationId,
    "webhookEscrow",
...
test("S384 PSP operationId matches disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.equal(
    served.paths["/api/webhooks/psp"]?.post?.operationId,
    disk.paths["/api/webhooks/psp"]?.post?.operationId,
  );
  assert.equal(
    served.paths["/api/webhooks/psp"]?.post?.operationId,
    "webhookPspLegacy",
...
test("S385 all webhook POST operationIds match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const res = await GET();
  const served = (await res.json()) as typeof disk;
  const webhookPaths = Object.keys(disk.paths)
    .filter((p) => p.startsWith("/api/webhooks/") && disk.paths[p]?.post)
    .sort();
  assert.ok(webhookPaths.length >= 8, `expected webhook POSTs, got ${webhookPaths.length}`);
  for (const p of webhookPaths) {
    assert.equal(
...
test("S436 webhook GET challenge documented", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  assert.deepEqual(served.paths[path]?.get, disk.paths[path]?.get);
  assert.equal(served.paths[path]?.get?.operationId, "webhookWhatsappChallenge");
  assert.equal(
    served.paths[path]?.get?.responses?.["200"]?.description,
    "Challenge echo",
  );
  assert.equal(
    served.paths[path]?.get?.responses?.["403"]?.description,
    "Verify token mismatch",
  );
  const names = (served.paths[path]?.get?.parameters ?? []).map((p) => p.name);
  for (const n of ["hub.mode", "hub.verify_token", "hub.challenge"]) {
    assert.ok(names.includes(n), `missing param ${n}`);
  }
});

test("S437 ContiPay responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/webhooks/contipay"]?.post?.responses,
    disk.paths["/api/webhooks/contipay"]?.post?.responses,
  );
  assert.deepEqual(
    Object.keys(
      served.paths["/api/webhooks/contipay"]?.post?.responses ?? {},
...
test("S438 Paynow responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/webhooks/paynow"]?.post?.responses,
    disk.paths["/api/webhooks/paynow"]?.post?.responses,
  );
  assert.deepEqual(
    Object.keys(
      served.paths["/api/webhooks/paynow"]?.post?.responses ?? {},
...
test("S439 tags count locked", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const res = await GET();
  const served = (await res.json()) as typeof disk;
  assert.equal((served.tags ?? []).length, (disk.tags ?? []).length);
  assert.ok((disk.tags ?? []).length >= 3);
});

test("S440 servers count locked", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const res = await GET();
  const served = (await res.json()) as typeof disk;
  assert.equal((served.servers ?? []).length, (disk.servers ?? []).length);
  assert.equal((served.servers ?? []).length, 1);
});

test("S441 EcoCash responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/webhooks/ecocash"]?.post?.responses,
    disk.paths["/api/webhooks/ecocash"]?.post?.responses,
  );
});

test("S442 FDMS webhook responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/webhooks/fdms"]?.post?.responses,
    disk.paths["/api/webhooks/fdms"]?.post?.responses,
  );
});

test("S443 WhatsApp POST responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/webhooks/whatsapp"]?.post?.responses,
    disk.paths["/api/webhooks/whatsapp"]?.post?.responses,
  );
});

test("S444 health integrations responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/health/integrations"]?.get?.responses,
    disk.paths["/api/health/integrations"]?.get?.responses,
  );
});

test("S445 openapi path responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/openapi"]?.get?.responses,
    disk.paths["/api/openapi"]?.get?.responses,
  );
});

test("S446 PayPal responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/webhooks/paypal"]?.post?.responses,
    disk.paths["/api/webhooks/paypal"]?.post?.responses,
  );
});

test("S447 Escrow responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/webhooks/escrow"]?.post?.responses,
    disk.paths["/api/webhooks/escrow"]?.post?.responses,
  );
});

test("S448 PSP responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/webhooks/psp"]?.post?.responses,
    disk.paths["/api/webhooks/psp"]?.post?.responses,
  );
});

test("S449 admin FDMS day responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const res = await GET();
  const served = (await res.json()) as typeof disk;
  const path = "/api/admin/fdms/day";
  assert.deepEqual(
    served.paths[path]?.get?.responses,
    disk.paths[path]?.get?.responses,
  );
  assert.deepEqual(
    served.paths[path]?.post?.responses,
    disk.paths[path]?.post?.responses,
  );
});

test("S450 admin money outbox responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const res = await GET();
  const served = (await res.json()) as typeof disk;
  const path = "/api/admin/money/outbox";
  assert.deepEqual(
    served.paths[path]?.get?.responses,
    disk.paths[path]?.get?.responses,
  );
...
test("S451 admin daily-zig responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const res = await GET();
  const served = (await res.json()) as typeof disk;
  const path = "/api/admin/fx/daily-zig";
  assert.deepEqual(
    served.paths[path]?.get?.responses,
    disk.paths[path]?.get?.responses,
  );
...
test("S452 all webhook POST responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  for (const p of posts) {
    assert.deepEqual(
      served.paths[p]?.post?.responses,
      disk.paths[p]?.post?.responses,
      `${p} responses`,
    );
  }
});

test("S453 all admin path responses match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
...
test("S454 ContiPay parameters match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/webhooks/contipay"]?.post?.parameters,
    disk.paths["/api/webhooks/contipay"]?.post?.parameters,
  );
});

test("S455 WhatsApp POST parameters match disk", async () => {
  const { readFileSync } = await import("node:fs");
  const { join } = await import("node:path");
  const disk = JSON.parse(
    readFileSync(
  ...
  const served = (await res.json()) as typeof disk;
  assert.deepEqual(
    served.paths["/api/webhooks/whatsapp"]?.post?.parameters,
    disk.paths["/api/webhooks/whatsapp"]?.post?.parameters,
  );
});
