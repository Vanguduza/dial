...
- **Dial a Tech FixItNow nav + booking overlay (ENH-609)** — Navbar Diagnose `/tech/guide`, Emergency `/tech/emergency`, signed-in My jobs; How-it-works + home timeline drop card-pay copy for Diagnose → rate-card draft → EcoCash/COD Job Reserve. BookingModal shows labelled USD draft from `GET view=quote` / `draftTechQuote`. Job page EcoCash|COD CTAs (`POST action=pay`) reuse `@dial/payments` (fail-closed without Daily ZiG rate). Diagnose/Emergency restyled orange; contact Google Maps embed removed (D-44). AI still never writes payable amounts.
- **Dial a Tech OEM specialist routing** — `guidedIntake` returns structured `specialistHint` (no money, no tech ids). `@dial/jobs` matches registered `oemSpecialties` (Mercedes/Benz/MB aliases). Diagnose `/tech/guide` and BookingModal show **Recommended specialist**; Honda general is not forced. Audit: `docs/agent-audits/ai-capability-tech-specialist-routing-2026-08-17.md`.
- **Dial a Tech × FixItNow feature audit (ENH-609, plan only)** — [`docs/planning/DIAL_Tech_FixItNow_Feature_Audit.md`](docs/planning/DIAL_Tech_FixItNow_Feature_Audit.md): donor vs DIAL inventories, keep/replace/add/drop matrix, workflow overlay (diagnose, rate-card quotes, Job Reserve, EcoCash+COD, WHT). AI review first (`docs/agent-audits/ai-capability-tech-fixitnow-ux-2026-08-16.md`). No UI or `packages/ai` changes.
- **Dial a Tech FixItNow UI copy (ENH-609 / D-38)** — Founder: copy FixItNow customer Next.js UI, not redesign. `/tech` landing is the donor home (HeroCarousel + sections); `/tech/services`, PDP, `/tech/find-technicians` live; book posts `POST /api/tech/services` (session SoR, rate_card). Tailwind/shadcn scoped under `.fin-tech`. Honest: customer/technician dashboards, donor payments, and auth screens not ported. Rule: `.cursor/rules/dial-donor-identity.mdc` (UI copy override; no donor backend SoR).
- **Donor parity plan SoR (ENH-609 / D-38)** — [`docs/planning/DIAL_Donor_Parity_Roadmap.md`](docs/planning/DIAL_Donor_Parity_Roadmap.md): per-branch identities, screen matrices, P0–P7 build order, DoD. Founder 2026-08-16: copy FixItNow **UI**, not pattern-recreate. Rule `.cursor/rules/dial-donor-identity.mdc` bans a shared DIAL storefront / `AppShell` identity.
- **Dial a Tech thin vertical** — `/tech` book still wired to `POST /api/tech/services`. Catalogue + PDP + directory copied from FixItNow. Job pay EcoCash|COD is on `/tech/jobs/[id]` (fail-closed without Daily ZiG rate). Remaining: customer/technician dashboards, reviews.
- **Web design system + app shell** — `apps/gateway-web/src/app/globals.css` (tokens → radii/shadows/breakpoints/component chrome), `next/font` Fraunces + Source Sans 3 wired through `cssVariables()`, shared `AppShell` (sticky header + search, section tabs, mobile bottom nav, agency footer) and `ProductThumb` deterministic catalogue illustrations. Spare browse currently uses this chrome — **ENH-609 P0 rolls `AppShell` back** as cross-branch identity (Mercur B2C skin instead; DIAL APIs stay SoR).
- **Vercel preview deployment** — project `dial`; `.vercelignore`, root `vercel.json` (pnpm workspace install + `@dial/gateway-web` build), `scripts/vercel-set-root-directory.mjs`, `scripts/vercel-preview-bypass.mjs`, `apps/gateway-web/scripts/vercel-preview-smoke.mts`. Preview runs `DIAL_INTEGRATION_MODE=fixture`; no live keys (see `.cursor/rules/dial-credentials-after-testing.mdc`).
- **Credential lock rule** — `.cursor/rules/dial-credentials-after-testing.mdc`: live payment / WhatsApp / tax credentials are injected only after everything is tested and confirmed production-ready.

- **Phase 12 hardening prep (not G12)** — spare-orders IDOR lock (BUG-047); `phase12PrepOps.test.ts`; `g12-hardening-prep.mts` (`g12Claimed=false`); §5.16 dormancy register; G5 recon waits for MapLibre GL canvas (`data-testid=dial-map`).
...
- **Sign-in looked like a no-op on Vercel preview** — DialSession lived only in an in-process Map; after form 303 to `/home`, a different serverless isolate had no session and bounced to `/` with no error. Cookies are now HMAC-signed (`ds1.*`) so any isolate can restore the session; Set-Cookie also sets `Secure` in production (BUG-051).
- **Form sign-up showed raw JSON** — `POST /api/auth/sign-up` always returned JSON even for HTML form posts; browser never navigated to `/home`. Form posts now 303-redirect to a safe `next` (default `/home`) with the DialSession cookie, matching sign-in; JSON clients keep the API shape (BUG-050).
- **Production web build was impossible** — the gateway imported `@dial/worker-temporal` (and through it `@temporalio/worker`, which webpack cannot bundle); Temporal task-queue config/health moved to `@dial/shared`. Route and page files no longer export non-route symbols (`__testMoneyOutbox`, `__testPaynowPayments` → `src/lib/testHandles.ts`; `GROCERY_CART_COOKIE`; `sessionFromCookieHeader`), and `GET(req?: Request)` handlers now take a required `Request`. `next build` passes; `output: "standalone"` is now opt-in via `DIAL_BUILD_STANDALONE=1` so only the container image uses it.
- **Map tiles blocked in production** — CSP `connect-src` allowed only `'self'` plus localhost, so MapLibre could not fetch styles, glyphs or tiles from a deployed origin; https is now allowed and the localhost entries are dev-only.
...
- S454 Smoke: ContiPay parameters match disk.
- S455 Smoke: WhatsApp POST parameters match disk.
- S456 Smoke: Paynow parameters match disk.
- S457 Smoke: all webhook POST parameters match disk.
- S458 Smoke: requestBody refs WebhookOpaqueBody.
- S459 Smoke: admin security InternalApiSecret only.
- S460 Smoke: tags include webhooks health admin.
- S461 Smoke: Paynow Hash parameter locked.
- S462 Smoke: ContiPay signature header locked.
