...
| --- | --- | --- | --- |
| ENH-001 | **Ticket hygiene → first thin vertical** (E2a preferred or E1a) with DoD + owner before parallel trains | `in_progress` | E2a = [#1](https://github.com/Vanguduza/dial/issues/1); autonomous runbook active |
| ENH-006 | **Autonomous Dev Manager loop** — prefer/lock defaults; no founder wait; escalate only true OPENs | `accepted` | `docs/planning/DIAL_Dev_Manager_Autonomous_Runbook.md` |
...
| ENH-005 | **Development Prime** mandatory Build **harness** (`prime-agent`, MIT) — install before workspace bootstrap; hosts Dev Manager | `accepted` | **D-61** — Dev Manager = managerial authority throughout; no prod data path; not CI/`AGENTS.md` SoR; **reject** prod Prime adapter / alternate frameworks |
