...
| --- | --- | --- | --- |
| ENH-609 | **Per-branch donor identity (D-38 clarified)** — each website/native at par with its locked donor; shared customer `AppShell` / one DIAL storefront brand is **out** | `in_progress` | **Founder 2026-08-16: copy FixItNow UI, not redesign.** Tech `/tech` copied FixItNow public home + services + technicians, wired to DIAL `POST /api/tech/services`. **OEM specialist routing (2026-08-17).** **Nav + booking overlay (2026-08-17):** Diagnose/Emergency/My jobs in FixItNow Navbar; How-it-works Diagnose → EcoCash/COD Job Reserve; BookingModal rate-card **draft**; job pay EcoCash\|COD (fail-closed without Daily ZiG rate); `/tech/guide` + emergency orange-skinned; Google Maps embed dropped on contact. Not full donor parity (dashboards/reviews still open). SoR: [`docs/planning/DIAL_Donor_Parity_Roadmap.md`](docs/planning/DIAL_Donor_Parity_Roadmap.md). Remaining: P0 Spare AppShell rollback; then Spare Mercur, Grocery, Supplier, Account, Delivery, natives. Does not claim G3/G12 |
| ENH-610 | **Vercel preview environment** — hosted fixture-mode web testing before any live credentials | `in_progress` | Project `dial` deployed and smoked (33 routes, no 5xx). Open: preview is behind Vercel deployment protection, and Root Directory is not set (repo root carries `next` + `vercel.json` as a workaround) |
| ENH-013 | Self-hosted maps (Planetiler + tileserver + Nominatim/OSRM/VROOM) | `done` (local) | `docker-compose.maps.yml` + `DialMap`; live tiles still need `pnpm maps:prepare` mbtiles |
| ENH-604 | **Completion eng-safe prep Phases 3–12 (not G3–G12)** — native URL flavors, privacy/signing drafts, G3 HTTP probe, Temporal/maps/WHT/WA_FLOW/CC/hardening; admin K/G/P dormant shells; P4/P5 gateway fixture CI | `in_progress` | **Phase 12 prep current (not G12)**; AppSec/ops contracts + spare-orders IDOR; G12 blocked on ENH-011 staging cohort + open G3/G5/G6/G9 |
| ENH-602 | **Completion Phase 4 / G4** — durable supplier/Factory/Meili | `done` | G4 green 2026-08-16 — REST Factory→Meili `taskUid=65`; confirm-SLA; admin T+S+A `docs/ops/evidence/g4/` |
| ENH-605 | **Completion Phase 5 prep (not G5)** — Temporal compose + dispatch contracts | `in_progress` | Temporal + workflow history + durable POD; **admin MapLibre recon retracted until GL canvas evidence**; G5 open — ENH-013 + device POD |
| ENH-603 | **Completion Phase 6 prep (not G6)** — Pack checklist library 42 + tech Android evidence path | `in_progress` | 42 seeds + web book/emergency + D-56 audit + Kotlin/HTTP dogfood green; **device PNG open** (G3) |
| ENH-607 | **Completion Phase 8 / G8** — sandbox money matrix + webhook duplicate + FDMS day + WHT | `done` | G8 green 2026-08-16 — `g8-sandbox-money-matrix.mts` + `phase8PrepOps.test.ts`; live keys blocked_on_human |
| ENH-608 | **Completion Phase 9 prep (not G9)** — WA Flow EcoCash\|COD + fiscal channel=wa | `in_progress` | `phase9PrepOps` 6/6; probe fail-closed without WHATSAPP_*; live open ENH-021 |
| ENH-606 | **Completion Phase 7 prep (not G7)** — launch-critical + A–P admin ops recon | `in_progress` | Launch 30 PNGs green; extended A–P script landed; run against :3000 for G7 |
| ENH-601 | **Completion Phase 2 / G2** — Spare web E2E (durable order path + EcoCash\|COD) | `in_progress` | COD + Paynow durable shapes + B2B probes; EcoCash keys founder — G2 not exit |
| ENH-600 | **Completion Phase 1 / G1** — live platform foundation (Auth/DB/Meili/workers) | `done` | G1 green 2026-08-16; Auth GoTrue+cookie→`/api/auth/me`; Meili taskUid; RLS; workers fail-closed |
...
| ENH-030 | **API key-drop-in adapters** — Pack/Stitch HTTP shapes + fixture/sandbox/live mode; per-vendor webhooks | `done` | S91 + standing rule `dial-key-drop-in.mdc` + `docs/integrations/key-drop-in-readiness.md` (2026-08-16 harden) |
...
| ENH-092 | **Admin UI truncated health note** | `done` | S154 — parse note + truncate helper |
...
| ENH-097 | **Admin hint: HEALTH_NOTE_UI_MAX** | `done` | S159 — integrations + cost-health |
| ENH-098 | **README cites healthNoteUiMax** | `done` | S160 — x-dial-sor.healthNoteUiMax |
| ENH-099 | **.env.example HEALTH_NOTE_UI_MAX** | `done` | S161 — header cites truncation SoR |
| ENH-100 | **OpenAPI healthNoteUiMax export lock** | `done` | S162 — served fragment matches constant |
| ENH-101 | **buildIntegrationsHealthNote helper** | `done` | S163 — mode + labels note SoR |
| ENH-102 | **Health route uses note builder** | `done` | S164 — single note SoR |
| ENH-103 | **Document buildIntegrationsHealthNote** | `done` | S165 — README + OpenAPI note |
...
| ENH-108 | **OpenAPI healthNote export lock** | `done` | S170 — served fragment matches builder |
| ENH-109 | **Admin note-builder UI cross-link** | `done` | S171 — note-builder-sor-hint |
| ENH-110 | **README note-builder-sor-hint** | `done` | S172 — admin cross-link documented |
| ENH-111 | **OpenAPI info.description note builder** | `done` | S173 — buildIntegrationsHealthNote |
| ENH-112 | **Root README note-builder-sor-hint** | `done` | S174 — admin cross-link cited |
...
| ENH-117 | **Admin UI HINT_ID data-testid** | `done` | S179 — import constant for test id |
| ENH-118 | **OpenAPI noteBuilderHint export lock** | `done` | S180 — served fragment matches HINT_ID |
| ENH-119 | **README cites HINT_ID + DOCS** | `done` | S181 — integrations README |
| ENH-120 | **Root README HINT_ID + DOCS** | `done` | S182 — root README cites constants |
| ENH-121 | **OpenAPI noteBuilderDocs SoR** | `done` | S183 — x-dial-sor.noteBuilderDocs |
...
| ENH-126 | **Root README noteBuilderDocs** | `done` | S188 — root README |
| ENH-127 | **OpenAPI description noteBuilderDocs** | `done` | S189 — info.description |
| ENH-128 | **Sandbox ready=false incomplete probes** | `done` | S190 — no REDIS_URL |
| ENH-129 | **Live ready=false incomplete probes** | `done` | S191 — no REDIS_URL |
| ENH-130 | **OpenAPI x-dial-sor HINT+DOCS keys** | `done` | S192 — served SoR keys |
| ENH-131 | **Fixture ready≠groups configured** | `done` | S193 — ready=true incomplete groups |
| ENH-132 | **ready vs groups SoR docs** | `done` | S194 — README + OpenAPI |
| ENH-133 | **Health note mode variance** | `done` | S195 — fixture≠sandbox/live note |
| ENH-134 | **OpenAPI webhook sig+idempotency** | `done` | S196 — tag + x-dial-sor keys |
| ENH-135 | **OpenAPI webhook no-secret smoke** | `done` | S197 — ban sk_live/service_role/… |
| ENH-136 | **Served OpenAPI webhook SoR keys** | `done` | S198 — signature + idempotency |
| ENH-137 | **Root README ready≠groups** | `done` | S199 — readyVsGroups cite |
| ENH-138 | **.env.example ready≠groups** | `done` | S200 — readyVsGroups cite |
...
| ENH-143 | **Cost-health ready≠groups** | `done` | S205 — hint parity |
| ENH-144 | **OpenAPI readyVsGroupsHint** | `done` | S206 — HINT_ID key |
| ENH-145 | **README READY_VS_GROUPS_HINT** | `done` | S207 — docs cite constant |
...
| ENH-469 | **PD59 Assignment-event timeline** offer/reject/FIFO/override | `done` | Pack §9.5; D-45 |
...
| ENH-505 | **PD95 Spare facets + collections** | `done` | Pack §9.2 |
...
| ENH-521 | **PD111 Intake → book** | `done` | Pack §10 same job id |
| ENH-522 | **PD112 Web Paynow URL checkout** | `done` | Optional rail; D-57 EcoCash\|COD required |
| ENH-523 | **PD113 Formbricks + PostHog stubs** | `done` | T8 fail-closed; not money |
| ENH-524 | **PD114 Checklist catalog seed** | `done` | Library tranche ≠ full 42 |
| ENH-525 | **PD115 Spare order track timeline** | `done` | Pack §9.2 |
