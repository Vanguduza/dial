...
**Health `note` (S151/S152/S153/S156):** Response JSON `note` includes `groups labels=` followed by the ordered `INTEGRATION_ENV_GROUP_LABELS` tuple (comma-separated). OpenAPI `IntegrationsHealth.note` description documents the same contract — never echoes secret values. Admin UIs (`/admin/integrations`, `/admin/cost-health`) display the note via `truncateIntegrationsHealthNote` capped by `INTEGRATIONS_HEALTH_NOTE_UI_MAX` (full string remains on the element `title` / health JSON).
